    ...
    while (1) {
        ...
        { // update player's position; TODO: move this block to a more appropriate place
            ...
            for (size_t i=0; i<gs.monsters.size(); i++) { // update the distances from the player to each sprite
                gs.monsters[i].distance_to_player = std::sqrt(pow(gs.player.x - gs.monsters[i].x, 2) + pow(gs.player.y - gs.monsters[i].y, 2));
            }
            std::sort(gs.monsters.rbegin(), gs.monsters.rend()); // sort it from farthest to closest
        }

        render(fb, gs.map,gs.player,gs.tex_walls,gs.tex_monst,gs.monsters); // render the scene to the frambuffer
